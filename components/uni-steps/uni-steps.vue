		...
		<view :class="[direction==='column'?'uni-steps__column':'uni-steps__row']">
			<view :class="[direction==='column'?'uni-steps__column-text-container':'uni-steps__row-text-container']">
				<view v-for="(item,index) in options" :key="index" :class="[direction==='column'?'uni-steps__column-text':'uni-steps__row-text']">
					...
					<text :style="{color:deactiveColor}" :class="[direction==='column'?'uni-steps__column-title':'uni-steps__row-title']">{{item.createTime}}</text>
					<!-- index<=active?activeColor:deactiveColor -->
					<text :style="{color:deactiveColor}" :class="[direction==='column'?'uni-steps__column-desc':'uni-steps__row-desc']">{{item.orderState}}</text>
					<view v-if="item.imgUrl && item.type == 4 || item.imgUrl && item.type == 5">
						<image :src="item.imgUrl" style="height: 240rpx;width: 220rpx;"></image>
					</view>
				</view>
			</view>
